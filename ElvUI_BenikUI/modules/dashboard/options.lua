...
local function UpdateTokenOptions()
	...
	for i, v in ipairs(archyTables) do
		...
		local optionOrder = 1
		for _, id in ipairs(tableName) do
			E.Options.args.benikui.args.dashboards.args.panels.args.tokens.args.archyGroup.args[option] = {
				order = i,
				type = 'group',
				name = optionName,
				args = {
...
local function UpdateProfessionOptions()
	...
	if (prof1 or prof2 or archy or fishing or cooking) then
		...
		for _, id in pairs(proftable) do
			...
			if pname then
				E.Options.args.benikui.args.dashboards.args.panels.args.professions.args.choosePofessions.args[pname] = {
					order = optionOrder + 1,
					type = 'toggle',
