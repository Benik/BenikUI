		...
		for _, id in pairs(proftable) do
			...
			if pname then
				config.args.choosePofessions.args[pname] = {
					...
					set = function(info, value) E.private.benikui.dashboards.professions.choosePofessions[id] = value BUID:UpdateProfessions() end,
				}
			end
		end
	else
...
local function UpdateAllDashboards()
	...
	if db.professions.enable then BUID:UpdateProfessions() end
			...
			professions = {
				...
				args = {
					enable = {
						...
						set = function(info, value) db.professions[ info[#info] ] = value BUID:ToggleProfessions() end,
					},
					selectProfessions = {
						order = 2,
						type = 'group',
