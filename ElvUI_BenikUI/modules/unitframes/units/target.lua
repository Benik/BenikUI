	...
	do
		...
		if frame.USE_PORTRAIT and portrait.backdrop.style and E.db.benikui.unitframes.target.portraitStyle then
			...
			if not power.colorClass then
				if maxValue > 0 then
					if color then
						portrait.backdrop.style.color:SetColorTexture(color[1], color[2], color[3])
					else
						portrait.backdrop.style.color:SetColorTexture(altR, altG, altB)
					end
				else
					if color then
						portrait.backdrop.style.color:SetColorTexture(color[1] * mu, color[2] * mu, color[3] * mu)
					end
				end
			else
				...
				if maxValue > 0 then
					if isPlayer then
						portrait.backdrop.style.color:SetColorTexture(classColor.r, classColor.g, classColor.b)
					else
						if reaction then
							local tpet = ElvUF.colors.reaction[reaction]
							portrait.backdrop.style.color:SetColorTexture(tpet[1], tpet[2], tpet[3])
						end
					end
				else
					if reaction then
						local t = ElvUF.colors.reaction[reaction]
						portrait.backdrop.style.color:SetColorTexture(t[1] * mu, t[2] * mu, t[3] * mu)
					end
				end
			end	
		end
