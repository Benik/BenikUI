...
function mod:UpdateNotifierPositions(bar, option)
	local db = E.db.benikui.benikuiDatabars[option].notifiers
...
function mod:ToggleBackdrop(bar, option)
	local db = E.db.benikui.benikuiDatabars[option]
...
function mod:ApplyStyle(bar, option)
	...
	if bar.holder.style then
		bar.holder.style:SetShown(E.db.benikui.benikuiDatabars[option].buiStyle)
	end
end
